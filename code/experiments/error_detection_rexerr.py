...
def get_detection_prompt(candidate_report: str, level: str = 'coarse') -> tuple:
    """
    ...
    user_prompt = f"""REPORT TO EVALUATE:
{candidate_report}
...
def get_detection_result(
    ...
    """
    system_prompt, user_prompt = get_detection_prompt(candidate_report, level)
...
def run_error_detection_rexerr(args):
    ...
    if len(remaining_data) == 0:
        ...
        for idx, item in enumerate(remaining_data, 1):
            original_text = item[reference_field]  # RexErr: reference is the original
            item_id = item['id']
    ...
    output_filename = f"{model_name_clean}_error_detection.jsonl"
