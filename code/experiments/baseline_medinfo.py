"""
Baseline experiment for MedInfo dataset: Perturbation + rating pipeline.

Supports LLM-based perturbations (inject_critical_error, inject_noncritical_error)
and regex-based perturbations (change_dosage, remove_sentences, add_typos).
"""
...
def run_baseline_experiment(args):
    ...
    # Define perturbation types for MedInfo
    # LLM-based perturbations by default
    ...
    # Default to LLM, all available includes both
    default_perturbations_coarse = llm_perturbations_coarse
    default_perturbations_fine = llm_perturbations_fine
    ...
    for level in levels:
        ...
        # Handle dataset loading for MedInfo
        # Use full datasets for change_dosage and LLM perturbations, subsets for others
        ...
        print(f"Loaded full {level} dataset: {len(all_qa_pairs_full)} examples (for change_dosage only)")

        # Load subset (from paths which points to subset files)
        all_qa_pairs_subset = load_qa_data(data_path)
        print(f"Loaded {level} subset: {len(all_qa_pairs_subset)} examples (for all other perturbations)")

        # Create mapping: change_dosage uses full dataset, all others use subset
        perturbation_to_dataset = {
            'change_dosage': all_qa_pairs_full,
            'inject_critical_error': all_qa_pairs_subset,
            'inject_noncritical_error': all_qa_pairs_subset,
            ...
            for remove_pct in remove_pcts:
                for typo_prob in typo_probs:
                    ...
                    for qa_pair in qa_pairs_to_rate:
                        ...
                        # For regex perturbations: typo_probability, remove_pctd, change_counts
                        # For LLM perturbations: reasoning, error_details, level, model, etc.
                        metadata_keys = [
                            'typo_probability', 'remove_pctd', 'change_counts', 'skip_reason',
                            'reasoning', 'error_details', 'parsed_successfully', 'num_changes',
                            'level', 'model', 'error', 'raw_response'
                        ]
