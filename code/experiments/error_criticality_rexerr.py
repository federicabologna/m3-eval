"""
Error Criticality Classification Experiment for RexErr Dataset.
...
def run_error_criticality_rexerr(args):
    """Run error criticality classification on RexErr error detection results."""
    ...
    print(f"\\n{'='*80}")
    print("ERROR CRITICALITY CLASSIFICATION - REXERR")
    print(f"{'='*80}")
    ...
    detection_files = list(Path(detection_dir).glob('*_error_detection.jsonl'))

    if not detection_files:
        print(f"  Warning: No detection files found in {detection_dir}")
        return
    ...
    output_filename = f"{model_name_clean}_criticality.jsonl"
    ...
    if len(remaining_data) == 0:
        ...
        print(f"Processing {len(remaining_data)} remaining entries (out of {len(detection_data)})")
        ...
        print(f"\\n✓ Completed criticality classification")
        print(f"✓ Results saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run error criticality classification on RexErr')
