"""
Error Detection Experiment for MedInfo Dataset (Reference-Based).

Ask models to detect medication errors in perturbed answers by comparing them to the
...
- Dosage, administration, safety, and clinical accuracy changes
"""
...
def get_detection_prompt(question: str, candidate_answer: str, reference_answer: str, level: str = 'coarse') -> tuple:
    """
    Build detection prompt for medication Q&A.

    Args:
        question: Medical question
        candidate_answer: Perturbed answer to evaluate
        reference_answer: Original correct answer
    ...
    prompt_file = f'error_detection_with_reference_medinfo_{level}_system.txt'
...
REFERENCE {text_type.upper()} (Correct):
{reference_answer}

CANDIDATE {text_type.upper()} (To Evaluate):
{candidate_answer}

Does the candidate {text_type} contain any medication errors or safety violations compared to the reference {text_type}? If yes, identify ALL errors.
...
def get_detection_result(
    ...
    """
    system_prompt, user_prompt = get_detection_prompt(question, candidate_answer, reference_answer, level)
...
def run_error_detection_with_reference_medinfo(args):
    """Run error detection experiment on MedInfo dataset with reference."""
    ...
    print(f"\n{'='*80}")
    print("ERROR DETECTION EXPERIMENT - MEDINFO (REFERENCE-BASED)")
    print(f"{'='*80}")
    print(f"Perturbations: {', '.join(perturbations_to_run)}")
    print("Testing error detection by comparing to original answer")
    ...
    for level in levels:
        ...
        # First, process original answers ONCE (control - comparing answer to itself)
        print(f"\n{'='*80}")
        ...
        output_filename = f"original_{level}_{model_name_clean}_error_detection_with_reference.jsonl"
        ...
        if len(remaining_data) == 0:
            ...
            for idx, item in enumerate(remaining_data, 1):
                ...
                # Compare original answer to itself (should find no errors)
                detection_result = get_detection_result(
                    question, original_answer, original_answer,
                    model=args.model,
                    level=level,
                    max_retries=3
                )
        ...
        for perturbation_name in perturbations_to_run:
            ...
            output_filename = f"{perturbation_name}_{level}_{model_name_clean}_error_detection_with_reference.jsonl"
            ...
            if len(remaining_data) == 0:
                ...
                for idx, item in enumerate(remaining_data, 1):
                    ...
                    original_answer = item['answer']  # Original answer from the item
                    item_id = item[id_key]
                    ...
                    # Compare perturbed answer to original answer
                    detection_result = get_detection_result(
                        question, perturbed_answer, original_answer,
                        model=args.model,
                        level=level,
                        max_retries=3
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run error detection experiment on MedInfo (reference-based)')
